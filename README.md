...
-----------------------
2016-4-4更新
百度前端技术学院的任务中有一个根据数据生成条形统计图的任务。
刚好可以用svg来实现，也巩固一下svg的相关知识。
具体实现代码在test_svg.html中。
在实现的时候，通过document.createElement去创建rect标签时，发现标签无法显示。百度之后发现，不能用createElement，应该用XML中创建DOM的方法，也就是createElementNS();
...
所以创建一个rect标签：
```javascript
var rectElement = document.createElementNS("www.w3.org/2000/svg","rect");
```

显示效果如下：

![test_svg.png](img/test_svg.png)
